...
describe("tests with definitions containing errors to see what's reported", () => {
      ...
      expected_errors: [
        {
          ...
          Message:
            '/States/PassState/InputPath is invalid. must match format "asl_path"',
          schemaError: {
      ...
      expected_errors: [
        {
          "Error code": StateMachineErrorCode.SchemaValidationFailed,
          Message:
            "/States/PassState is invalid. must match exactly one schema in oneOf",
          schemaError: {
      ...
      expected_errors: [
        {
          "Error code": StateMachineErrorCode.SchemaValidationFailed,
          Message:
            '/States/Hello, World/Parameters/lorem.$ is invalid. must match format "asl_payload_template"',
          schemaError: {
  ...
  test.each(inputs)("$file", (input) => {
    ...
    const definition = JSON.parse(
      fs.readFileSync(path.join(__dirname, "definitions", input.file), "utf-8")
    ) as StateMachine;
