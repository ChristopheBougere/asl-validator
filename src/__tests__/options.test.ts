...
describe("show how the options impact validation", () => {
    ...
    const loadDef = (file: string): StateMachine => {
        return JSON.parse(fs.readFileSync(path.join(__dirname, 'definitions', file), "utf-8")) as StateMachine;
    }
    ...
    describe("checkPaths", () => {
        ...
        it.each(
            [
                "invalid-json-path.json",
                "invalid-payload-template.asl.json"
            ]
        )("%s should not report path error", (file) => {
